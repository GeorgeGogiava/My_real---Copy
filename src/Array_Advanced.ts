...
let newArr: number[] = [];
yq.forEach((value: number, idx: number) => {
  newArr[idx] = value * 10;
});
console.log(newArr); //[ 40, 50, 70, 100 ]

// მასივში ყველაზე 5-ის დამატება
yq.forEach((element, idx: number) => {
  yq[idx] = element + 5;
});
...
const arr5: number[] = [15, 45, 80, 189, 20, 60];
let arr6: number[] = arr5.filter((value) => value % 10 === 0);
...
const uniqueObjects = Array.from(
  new Set(arr11.map((obj) => JSON.stringify(obj)))
...
class Person implements IPerson {
  constructor(
    public name: string,
    public age: number,
    public isGeorgian: boolean,
    public birthday: dayjs.Dayjs
  ) {}
  toString(): string {
    let geor: string = this.isGeorgian ? "ქართველი" : "არაქართველი";
    return `${this.name}, ${
      this.age
    } old, ${geor}, დაბ. დღე - ${ggDates.formatDate(this.birthday)} `;
  }
}
...
// Deserialize json
const json2 =
  '[{"name":"John","age":47,"isGeorgian":false,"birthday":"2058-01-31T20:00:00.000Z"},{"name":"Kakha","age":48,"isGeorgian":true,"birthday":"1974-08-19T20:00:00.000Z"}]';
const data = JSON.parse(json2);
const people: Person[] = data.map(
  (obj: any) => new Person(obj.name, obj.age, obj.isGeorgian, obj.birthday)
);
console.log(people);
...
let arr20: number[] = [10, 2, 3, 4, 5, 6, 7];
...
const fruitCounts: FruitCount[] = fruits.reduce(
  (accumulator: FruitCount[], currentValue: string) => {
    const index = accumulator.findIndex(
      (element: FruitCount) => element.fruit === currentValue
    );
...
interface CarInstances {
  [car: string]: number;
}

let instances: CarInstances = myCars.reduce(function (
  allCars: CarInstances,
  car: string
) {
  if (car in allCars) {
    allCars[car]++;
